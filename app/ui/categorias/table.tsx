...
import { fetchFilteredCategorias } from '@/modules/categorias/data/categorias';
import type { CategoriaComPai } from '@/modules/categorias/types/categoria';
...
function DesktopCategoriasTable({ categorias }: { categorias: CategoriaComPai[] }) {
  return (
...
export default async function CategoriasTable({
  ...
  const categoriasComNomePai = await fetchFilteredCategorias(currentPage, queryCategoria);

  if (!categoriasComNomePai || categoriasComNomePai.length === 0) {
    return <p className='mt-6 text-center text-gray-500'>Nenhuma categoria encontrada.</p>;
  }
